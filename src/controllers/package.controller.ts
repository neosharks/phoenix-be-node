...
class _PackageController {
  ...
  async createOnePackage(req: Request, res: Response) {
    try {
      ...
      if (packageIndex !== -1)
        return res.status(400).send({ message: errorMessage.DUPLICATE_ENTRY });
  ...
  async updatePackage(req: Request, res: Response) {
    try {
      ...
      if (!foundPackage) return res.status(400).send({ message: errorMessage.NOT_FOUND });
  ...
  async buyPackage(req: Request, res: Response) {
    try {
      ...
      if (!foundPackage)
        return res.status(404).send({ message: errorMessage.NOT_FOUND, info: "Package not found" });
      const { tier, creatorId } = foundPackage;
  ...
  async createManyTier(req: Request, res: Response) {
    try {
      ...
      if (!tiers || tiers.length < 1)
        res.status(400).send({ message: errorMessage.MISSING_PARAMS, info: "Provide tiers" });
