      ...
      try {
        ...
        if (foundUser)
          return res
            ...
            .json({ message: api_constant_1.errorMessage.USER_EXISTS });
        const saltRounds = 10;
        ...
        if (body.referralUsername) {
          const referralUser = yield user_service_1.UserService.getOneUser({
            username: body.referralUsername,
          });
          if (referralUser) {
        ...
        const created = yield user_service_1.UserService.createOneUser(
          isCreator
            ? Object.assign(Object.assign({}, body), {
                profileImage,
                isCreator: true,
                role: ["PATRON", "CREATOR"],
              })
            : Object.assign(Object.assign({}, body), { profileImage }),
        );
        ...
        return res
          .status(201)
          .json({ messge: api_constant_1.successMessages.CREATED, accessToken, user: created });
      } catch (err) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (!number)
          return res
            .status(api_constant_1.errorCode.FORBIDDEN)
            .json({ message: api_constant_1.errorMessage.MISSING_PARAMS });
        ...
        if (checkRes) {
          ...
          if (!smsRes)
            return res
              ...
              .json({ message: api_constant_1.errorMessage.SMS_ISSUE });
        }
        ...
        if (foundUser) {
          ...
          yield user_service_1.UserService.updateOneUser(
            { phoneNumber: number },
            Object.assign(Object.assign({}, commonProps), {
              verificationCodeAttempts: foundUser.verificationCodeAttempts + 1,
            }),
          );
        } else {
          if (referralUsername) {
          ...
          const randomNum = (Math.random() * 25) | 1;
          const profileImage = `https://phoenix-test-bucket9415.s3.ap-south-1.amazonaws.com/avatars/avatar_${randomNum}.jpg`;
          yield user_service_1.UserService.createOneUser(
            Object.assign(
              { username, phoneNumber: number, profileImage, verificationCodeAttempts: 1 },
              commonProps,
            ),
          );
        }
    ...
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (!foundUser)
          return res
            .status(api_constant_1.errorCode.GENERIC)
            .json({ message: api_constant_1.errorMessage.USER_NOT_FOUND });
        const { verificationCodeTimestamp, verificationCodeAttempts } = foundUser;
        if (verificationCodeTimestamp && verificationCodeAttempts > 1) {
          const fiveMinutesAgo = new Date();
          fiveMinutesAgo.setMinutes(fiveMinutesAgo.getMinutes() - 5);
          ...
          if (dateVC < fiveMinutesAgo)
            return res
              .status(api_constant_1.errorCode.GENERIC)
              .json({
                message: api_constant_1.errorMessage.NOT_ALLOWED,
      ...
      try {
        ...
        if (!foundUser)
          return res
            ...
            .json({ message: api_constant_1.errorMessage.NOT_FOUND });
        ...
        yield (0,
        email_core_1.default)(email, `OTP to Reset your password | ${foundUser.username}`, "FORGET_PASSWORD", {
          code,
    ...
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (parseInt(foundUser.verificationCode) !== parseInt(code))
          return res
            .status(api_constant_1.errorCode.UNAUTHORISED)
            .json({ message: api_constant_1.errorMessage.INCORRECT_DATA });
        const saltRounds = 10;
        const salt = yield bcrypt_1.default.genSaltSync(saltRounds);
  ...
  resetPassword(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        return res
          .status(200)
          .json({ message: api_constant_1.successMessages.SUCCESS, accessToken });
      } catch (error) {
        console.log("ERROR: ", error);
        return res
          .status(api_constant_1.errorCode.INTERNAL_SERVER)
    ...
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (!foundUser.phoneVerified) {
          createdUser = yield user_service_1.UserService.updateOneUser(
            { phoneNumber: number },
            Object.assign({ phoneVerified: true }, commonProps),
          );
        } else createdUser = yield user_service_1.UserService.updateOneUser({ phoneNumber: number }, Object.assign({}, commonProps));
      ...
      try {
        ...
        if (!googleAccessToken)
          return res
            .status(api_constant_1.errorCode.FORBIDDEN)
            .json({ message: api_constant_1.errorMessage.MISSING_PARAMS });
        const FetchResponse = yield axios_1.default.get(
